...
func _create_slot(index: int) -> SlotUI:
	...
	face_container.custom_minimum_size = Vector2(FACE_SIZE, FACE_SIZE)
	face_container.clip_contents = true
	slot.container.add_child(face_container)
	...
	face_container.add_child(slot.face)
	...
	face_container.add_child(slot.damage_overlay)
